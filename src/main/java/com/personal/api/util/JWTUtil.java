...
import com.personal.api.service.impl.AppUserServiceImpl;
...
public class JWTUtil {
    ...
    public static boolean isTokenExpired(String token) {
        return DateUtils.getLocalDateTime(JWT.decode(token).getExpiresAt()).isBefore(LocalDateTime.now());
    }
    ...
    public static String sign(String userId, String secret, long expire) {
        try {
            ...
            Algorithm algorithm = Algorithm.HMAC256(secret);
            return JWT.create().withClaim(jwtConfigProperties.getUserPrimaryKey(), userId).withExpiresAt(new Date(System.currentTimeMillis() + expire)).sign(algorithm);
